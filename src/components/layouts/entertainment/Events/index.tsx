...
export default function Events() {
  ...
  const { events = [], loading, error, toggleLike, toggleBookmark } = useEvents();
    ...
    <section className="max-w-[1120px] w-full mx-auto flex flex-col gap-[50px] py-12 md:py-[100px] px-6 lg:px-0">
      ...
      {view === "grid" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          {loading ? (
            <p className="text-gray-500 text-center col-span-2">Loading events...</p>
          ) : error ? (
            ...
            filteredEvents.map((event) => (
              <EventCard 
                key={event.id} 
                event={event} 
                onLike={() => event.id && toggleLike(event.id)}
                onBookmark={() => event.id && toggleBookmark(event.id)}
                isAuthenticated={isAuthenticated}
              />
            ))
          ) : (
            <p className="text-gray-500 text-center col-span-2">
              No events found {selectedMonth ? `for ${selectedMonth}` : ''}.
            </p>
          )}
        </div>
      ) : (
