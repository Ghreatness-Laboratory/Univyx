...
export default function SignUp() {
      ...
      if (error.response?.data?.errors) {
        ...
        const errorMessages = Object.entries(errors)
          .map(([field, messages]: [string, any]) => {
            ...
            return `${field.charAt(0).toUpperCase() + field.slice(1)}: ${messageArray.join(', ')}`;
          })
          .join('. ');
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      setApiError('Google OAuth is not configured yet. Please use email signup.');
      
      // TODO: Implement Google OAuth
      // This would typically involve:
      // 1. Loading Google OAuth SDK
      // 2. Getting Google token
      // 3. Calling googleAuth(token)
    ...
    <div data-testid="sign-up-page" className="w-full md:max-w-[90%] mx-auto">
      ...
      <div className="flex max-sm:flex-col items-center justify-center gap-[13px] text-[#FCFCFC] text-sm font-semibold leading-4">
        <button
          ...
          className="flex gap-1 items-center justify-center py-4 rounded-[5.35px] w-full bg-primary text-[#FCFCFC] border border-[#64748B] shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)] cursor-pointer disabled:opacity-50"
        ...
        >
          ...
          <p>{googleLoading ? 'Connecting...' : 'Sign Up with Google'}</p>
        </button>
        <button
